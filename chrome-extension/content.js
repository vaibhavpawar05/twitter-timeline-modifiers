...
async function saveInteraction(tweetId, action, emoji, html) {
  ...
  const existingInteractions = currentData["tweetInteractions-v1"]
    ? new Map(JSON.parse(currentData["tweetInteractions-v1"]))
    : new Map();
...
function modifyTweetElements(tweet) {
  ...
  const tweetId = Array.from(links).filter((val) =>
    val.href.includes("status"),
  )[0]?.href;
  if (!tweetId) return;

  // Hide ads (required on non-Brave browsers)
  const spans = tweet.querySelectorAll("span");
  const isAd =
    Array.from(spans).filter((val) => val.textContent === "Ad").length > 0;

  if (isAd) {
    //console.log(tweetId, "ad found");
    tweet.setAttribute("data-isad", "true");
  }
